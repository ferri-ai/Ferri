...
import (
	...
	"ferri/engine/core/constants"
	"ferri/engine/core/logger"
...
type AgentLoop struct {
	...
	cleanupFn      func() // Optional cleanup called on Stop (used by mobile to close owned bus)
	sessionMu      sync.Map            // map[string]*sync.Mutex — per-session locks
	drainCancel    context.CancelFunc  // Mobile only: stops bus drain when channels activate

	// Optional callbacks for tool execution events (used by mobile bridge for UI)
	OnToolStart func(toolName string, args map[string]interface{})
	OnToolEnd   func(toolName string, args map[string]interface{}, result string, elapsed time.Duration, isError bool)
}

// getSessionMu returns (or creates) a mutex for the given session key.
func (al *AgentLoop) getSessionMu(key string) *sync.Mutex {
	v, _ := al.sessionMu.LoadOrStore(key, &sync.Mutex{})
	return v.(*sync.Mutex)
}
...
func (al *AgentLoop) UnregisterTool(name string) {
	al.tools.Unregister(name)
}

// GetBus returns the message bus for external channel consumers.
// The returned bus is shared; callers must not close it.
func (al *AgentLoop) GetBus() *bus.MessageBus { return al.bus }

// GetWorkspace returns the workspace path for this agent loop.
func (al *AgentLoop) GetWorkspace() string { return al.workspace }

// StopDraining cancels the mobile bus drain goroutine.
// Called when channels are enabled and the bus needs real consumers.
func (al *AgentLoop) StopDraining() {
	cancel := al.drainCancel
	al.drainCancel = nil
	if cancel != nil {
		cancel()
	}
...
func (al *AgentLoop) ProcessDirect(ctx context.Context, content, sessionKey string) (string, error) {
	return al.ProcessDirectWithChannel(ctx, content, sessionKey, "chat", "direct")
}
...
func (al *AgentLoop) processMessage(ctx context.Context, msg bus.InboundMessage) (string, error) {
	mu := al.getSessionMu(msg.SessionKey)
	mu.Lock()
	defer mu.Unlock()
...
func (al *AgentLoop) runAgentLoop(ctx context.Context, opts processOptions) (string, error) {
	...
	// 2b. Inject cron-specific instruction for scheduled job processing
	if strings.HasPrefix(opts.SessionKey, "cron-") && len(messages) > 0 && messages[0].Role == "system" {
		messages[0].Content += "\n\n## Scheduled Task Instructions\nThis message is from a scheduled cron job. After executing any tools, you MUST provide a clear, natural-language summary of the results for the user. Never return an empty response — always describe what you found or did."
	}
...
func (al *AgentLoop) runLLMIteration(ctx context.Context, messages []providers.Message, opts processOptions) (string, int, error) {
	...
	var lastToolKey string // "toolName:argsJSON" — detects same-tool loops
	sameToolCount := 0
	const maxSameToolCalls = 3 // Break if same tool+args called this many times in a row
	...
	for iteration < al.maxIterations {
		...
		// Detect same-tool loops: if the LLM calls the exact same tool with
		// the same args multiple times in a row, break to prevent looping.
		if len(response.ToolCalls) == 1 {
			tc := response.ToolCalls[0]
			argsJSON, _ := json.Marshal(tc.Arguments)
			toolKey := tc.Name + ":" + string(argsJSON)
			if toolKey == lastToolKey {
				sameToolCount++
				if sameToolCount >= maxSameToolCalls {
					logger.WarnCF("agent", "Same tool called repeatedly, breaking loop",
						map[string]interface{}{
							"tool":      tc.Name,
							"count":     sameToolCount,
							"iteration": iteration,
						})
					// Use any content the LLM produced alongside the tool call
					if response.Content != "" {
						finalContent = response.Content
					}
					break
		...
		for _, tc := range response.ToolCalls {
			...
			if al.OnToolStart != nil {
				al.OnToolStart(tc.Name, tc.Arguments)
			}
			...
			if al.OnToolEnd != nil {
				...
				isErr := toolResult.Err != nil
				if isErr {
					resultPreview = toolResult.Err.Error()
				}
				al.OnToolEnd(tc.Name, tc.Arguments, resultPreview, elapsed, isErr)
			}
	...
	if len(newHistory) > 20 || tokenEstimate > threshold {
		if _, loading := al.summarizing.LoadOrStore(sessionKey, true); !loading {
			go func() {
				...
				logger.InfoCF("agent", "Session history threshold reached, summarizing", map[string]interface{}{
...
func (al *AgentLoop) GetSessions() *session.SessionManager {
	return al.sessions
}

// GetContextBuilder returns the context builder for accessing skills and other context.
func (al *AgentLoop) GetContextBuilder() *ContextBuilder {
	return al.contextBuilder
}
...
func (al *AgentLoop) handleCommand(ctx context.Context, msg bus.InboundMessage) (string, bool) {
	...
	case "/switch":
		...
		case "channel":
			...
			if _, exists := al.channelManager.GetChannel(value); !exists && value != "chat" {
