...
// InternalMessageHandler is called when an outbound message targets an internal
// channel (cli, system, subagent). On mobile, the bridge sets this to forward
// such messages to Dart as channel_response events.
type InternalMessageHandler func(msg bus.OutboundMessage)
...
type Manager struct {
	...
	onInternalMessage InternalMessageHandler
	mu                sync.RWMutex
}
...
func (m *Manager) dispatchOutbound(ctx context.Context) {
	...
	for {
		...
		default:
			...
			// Forward internal channels to the registered handler (if any)
			if constants.IsInternalChannel(msg.Channel) {
				m.mu.RLock()
				handler := m.onInternalMessage
				m.mu.RUnlock()
				if handler != nil {
					handler(msg)
				}
...
// SetInternalMessageHandler sets the callback for internal channel messages.
func (m *Manager) SetInternalMessageHandler(handler InternalMessageHandler) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.onInternalMessage = handler
}
...
// StartDispatcher starts only the outbound message dispatcher goroutine.
// Use this instead of StartAll when channels are managed dynamically
// (registered and started individually rather than all at once).
func (m *Manager) StartDispatcher(ctx context.Context) {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	logger.InfoC("channels", "Outbound dispatcher started (dynamic mode)")
}
