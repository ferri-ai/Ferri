...
func (cs *CronService) computeNextRun(schedule *CronSchedule, nowMS int64) *int64 {
	...
	if schedule.Kind == "geofence" {
		// Geofence jobs have no time-based next run; they are triggered externally
		return nil
	}
...
// TriggerJob manually triggers a geofence job by ID.
// Only enabled jobs with schedule kind "geofence" can be triggered.
func (cs *CronService) TriggerJob(jobID string) error {
	...
	if !found.Enabled {
		return fmt.Errorf("job %s is not enabled", jobID)
	}
	if found.Schedule.Kind != "geofence" {
		return fmt.Errorf("job %s is not a geofence job (kind=%s)", jobID, found.Schedule.Kind)
	}
