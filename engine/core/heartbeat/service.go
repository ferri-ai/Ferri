...
// SetEnabled sets the enabled flag (does not start/stop the service).
...
// SetInterval updates the heartbeat interval. Takes effect on next Start().
...
// GetIntervalMinutes returns the current interval in minutes.
func (hs *HeartbeatService) GetIntervalMinutes() int {
	hs.mu.RLock()
	defer hs.mu.RUnlock()
	return int(hs.interval.Minutes())
}
