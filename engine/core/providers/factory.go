...
func createClaudeAuthProvider(apiBase string) (LLMProvider, error) {
	...
	if cred == nil {
		return nil, fmt.Errorf("no credentials for anthropic. Configure your API key in settings.")
	}
...
func createCodexAuthProvider(enableWebSearch bool) (LLMProvider, error) {
	...
	if cred == nil {
		return nil, fmt.Errorf("no credentials for openai. Configure your API key in settings.")
	}
