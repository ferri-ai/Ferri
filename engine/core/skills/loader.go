...
type SkillsLoader struct {
	...
	globalSkills    string // 全局 skills (~/.ferri/skills)
...
func NewSkillsLoader(workspace string, globalSkills string, builtinSkills string) *SkillsLoader {
	return &SkillsLoader{
		...
		globalSkills:    globalSkills, // ~/.ferri/skills
...
func (sl *SkillsLoader) ListSkills() []SkillInfo {
	...
	// 全局 skills (~/.ferri/skills) - 被 workspace skills 覆盖
	if sl.globalSkills != "" {
...
func (sl *SkillsLoader) LoadSkill(name string) (string, bool) {
	...
	// 2. 其次从全局 skills 加载 (~/.ferri/skills)
	if sl.globalSkills != "" {
...
func (sl *SkillsLoader) extractFrontmatter(content string) string {
	...
	// Non-greedy (.*?) to match the FIRST closing --- (not one inside body content)
